...
def filter_raw(df, customer=None, vkorg=None, auart=None, matkl=None, werks=None,
    ...
    if customer and customer != "ALL":
        ...
        elif COL_KUNNR in df.columns:
            df = df[df[COL_KUNNR].astype(str) == customer]
    ...
    if matkl and matkl != "ALL":
        # Prefer MATKL, fall back to MATNR
        if COL_MATKL in df.columns and df[COL_MATKL].notna().any():
            df = df[df[COL_MATKL].astype(str) == matkl]
        elif COL_MATNR in df.columns:
            df = df[df[COL_MATNR].astype(str) == matkl]
...
def get_filters(
    ...
    def vals(col, fallback=None):
        target = col if (col in d.columns and d[col].notna().any()) else fallback
        if target and target in d.columns:
            return ["ALL"] + sorted([str(x) for x in d[target].dropna().unique()])
        return ["ALL"]

    return {
        "case_ids":  vals(COL_CASE),
        "customers": vals(COL_CUSTOMER, COL_KUNNR),
        "vkorgs":    vals(COL_VKORG),
        "auarts":    vals(COL_AUART),
        "matkls":    vals(COL_MATKL, COL_MATNR),
        "werkss":    vals(COL_WERKS),
        "vtwegs":    vals(COL_VTWEG),
...
def get_chart(
    ...
    col_map = {
        "customer": (COL_CUSTOMER, COL_KUNNR),
        "auart":    (COL_AUART, None),
        "matkl":    (COL_MATKL, COL_MATNR),
        "ernam":    (COL_ERNAM, None),
        "vkorg":    (COL_VKORG, None),
        "vtweg":    (COL_VTWEG, None),
        "spart":    (COL_SPART, None),
    }
    if chart_name in col_map:
        primary, fallback = col_map[chart_name]
        c_name = primary if (primary in d.columns and d[primary].notna().any()) else fallback
        
        if not c_name or c_name not in d.columns:
            return []
