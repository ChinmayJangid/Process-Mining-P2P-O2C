...
async def upload_raw_table(
    ...
    try:
        ...
        if mapping:
            # Check if mapping keys exist in df (case-insensitive check for robustness)
...
def build_event_log(username: str = Query("Unknown")):
    ...
    from p2p4 import USER_DFS, process_df, log_audit, _save_output_csv, register_transform_build, UPLOAD_DIR
    ...
    try:
        # Final pre-flight check of all required tables
        for t in REQUIRED_TABLES:
            ...
            missing = {c for c in required if c.upper() not in uploaded_cols}
            if missing:
                # Specific error prefix to trigger the "Fix Mapping" UI on frontend
        ...
        # Ensure 'Column mapping is incorrect' prefix is preserved for frontend logic
        if "Column mapping is incorrect" not in err_msg and any(t in err_msg for t in REQUIRED_TABLES):
            err_msg = f"Column mapping is incorrect: {err_msg}"
