...
const buildFlowMap = (bNodes, bEdges, setRfNodes, setRfEdges, dir) => {
  ...
  const nodes = (bNodes || []).map((n, index) => {
    const basePos = fixedMap[n.label];
    let rawPos;
    if (!basePos) {
      /* Other deviations / side activities → third lane */
      if (dir === 'LR') { rawPos = { x: otherX, y: 740 }; otherX += otherStep; }
      else { rawPos = { x: 2600, y: otherY }; otherY += otherStep; }
    } else {
      rawPos = basePos;
    }
    const pos = {
      x: rawPos.x + (index * 0.1),
      y: rawPos.y + (index * 0.1)
    };
    return {
          ...
          clonedNodes.forEach((cEl) => {
            ...
            if (oEl) {
              ...
              if (cEl.classList.contains('react-flow__background')) {
                cEl.style.setProperty('background-image', 'radial-gradient(#E1DFDD 1.5px, transparent 1.5px)', 'important');
