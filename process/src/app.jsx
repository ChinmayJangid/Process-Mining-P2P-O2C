...
const InteractiveProcessMap = ({ restrictToRight = false }) => {
  ...
  useEffect(() => {
    ...
    const updateDOM = (mouseX, mouseY) => {
      ...
      const dynamicNodes = baseNodes.current.map(node => {
        ...
        if (draggedNodeRef.current === node) {
          // If being dragged, stick directly to the cursor
          node.x = mouseXPct;
          node.y = mouseYPct;
        } else {
      ...
      dynamicNodes.forEach((node, i) => {
        if (nodesRef.current[i]) {
          ...
          // Store actual rendered coords for dragging hit detection
          baseNodes.current[i].cx = node.cx;
          baseNodes.current[i].cy = node.cy;
        }
      });
    ...
    const handleMouseDown = (e) => {
      // Don't grab nodes if clicking on interactive elements
      if (['INPUT', 'BUTTON', 'A', 'TEXTAREA', 'SELECT'].includes(e.target.tagName)) return;
      ...
      baseNodes.current.forEach(node => {
        const dx = mouseXPct - (node.cx || node.x);
      ...
      if (closest) {
        draggedNodeRef.current = closest;
    ...
    const loop = () => {
      // Made the easing slightly faster (0.15) so it feels snappy when dragging
