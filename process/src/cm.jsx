...
export default function CMDashboard({ currentUser, onSignOut, onBackHome }) {
  ...
  useEffect(() => {
    ...
    if (current && current.activeModule === 'cm') {
      if (current.step !== undefined) setStep(current.step);
    }

    const handlePopState = (event) => {
      if (event.state && event.state.activeModule === 'cm') {
  ...
  useEffect(() => {
    const current = window.history.state;
    if (!current || current.activeModule !== 'cm' || current.step !== step) {
      window.history.pushState({ activeModule: 'cm', step }, '');
    }
  }, [step]);
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
          ...
          link.download = `CM_Dashboard_Report_${new Date().toISOString().slice(0, 10)}.png`;
          link.href = url;
    ...
    <div ref={screenshotRef} style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {step === 'dashboard' && (
            <button
              id="cm-screenshot-btn"
              onClick={handleScreenshot}
              title="Download full report screenshot"
              style={{
                width: 34, height: 34, borderRadius: '50%',
                background: screenshotting ? 'rgba(247,99,12,0.5)' : 'linear-gradient(135deg, rgba(247,99,12,0.9) 0%, rgba(180,55,0,0.95) 100%)',
                border: '1.5px solid rgba(255,255,255,0.25)',
                boxShadow: screenshotting ? '0 0 0 3px rgba(247,99,12,0.4)' : '0 2px 12px rgba(247,99,12,0.45), 0 0 0 1px rgba(255,255,255,0.08)',
          ...
          <div className="process-ribbon-container">
            <div className="process-ribbon-content">
              {[...cmShortSteps, ...cmShortSteps, ...cmShortSteps, ...cmShortSteps].map((s, i) => {
                const isFirst = i % cmShortSteps.length === 0;
                return (
          ...
          <div style={{
            ...
            >
              <motion.div layout style={{
                ...
                <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={C.amber} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px 40px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, height: 600 }}>
              <ChartCard title="Change request status" loading={chartsLoading} skeletonType="pie" style={{ flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <ChartCard title="Change Cases List & Checklist" loading={chartsLoading} skeletonType="timeline">
              {selectedCaseId === 'ALL' ? (
