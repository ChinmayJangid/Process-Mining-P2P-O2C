...
export default function HRLDashboard({ currentUser, onSignOut, onBackHome }) {
  ...
  useEffect(() => {
    ...
    if (current && current.activeModule === 'hrl') {
      if (current.step !== undefined) setStep(current.step);
    }

    const handlePopState = (event) => {
      if (event.state && event.state.activeModule === 'hrl') {
  ...
  useEffect(() => {
    const current = window.history.state;
    if (!current || current.activeModule !== 'hrl' || current.step !== step) {
      window.history.pushState({ activeModule: 'hrl', step }, '');
    }
  }, [step]);
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
          ...
          link.download = `HRL_Dashboard_Report_${new Date().toISOString().slice(0, 10)}.png`;
          link.href = url;
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {step === 'dashboard' && !chartsLoading && (
            <button
              id="hrl-screenshot-btn"
              onClick={handleScreenshot}
              title="Download full report screenshot"
              style={{
                width: 34, height: 34, borderRadius: '50%',
                background: screenshotting ? 'rgba(209,52,56,0.5)' : 'linear-gradient(135deg, rgba(209,52,56,0.9) 0%, rgba(140,20,20,0.95) 100%)',
                border: '1.5px solid rgba(255,255,255,0.25)',
                boxShadow: screenshotting ? '0 0 0 3px rgba(209,52,56,0.4)' : '0 2px 12px rgba(209,52,56,0.45), 0 0 0 1px rgba(255,255,255,0.08)',
          ...
          <div className="process-ribbon-container">
            <div className="process-ribbon-content">
              {[...hrlShortSteps, ...hrlShortSteps, ...hrlShortSteps, ...hrlShortSteps].map((s, i) => {
                const isFirst = i % hrlShortSteps.length === 0;
                return (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <motion.div
              ...
              animate={{ borderColor: hoveredSide === 'build' ? C.red : 'rgba(255, 255, 255, 0.4)' }}
            >
            ...
            <motion.div
              ...
              onMouseLeave={() => setHoveredSide(null)}
              animate={{ borderColor: hoveredSide === 'csv' ? C.red : 'rgba(255, 255, 255, 0.4)' }}
            >
              <motion.div layout style={{
                width: '56px',
                height: '56px',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px 40px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, height: 600 }}>
              <ChartCard title="Separation Funnel status" loading={chartsLoading} skeletonType="pie" style={{ flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <ChartCard title="Monthly Separation Trend" loading={chartsLoading} skeletonType="line">
            ...
            <ChartCard title="Offboarding Cases List & Checklist" loading={chartsLoading} skeletonType="timeline">
              {selectedCaseId === 'ALL' ? (
